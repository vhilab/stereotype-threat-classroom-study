import viz, vizinput, viztask, time, vizact, random
...
ROOM = "DEV" # set to "PPT1" to use PPT1 -> this affects the tracking module used
AMBISONIC = False # set to True if running in PPT1
DATA = True # set to True if running (IMPORTANT)
FADE_RATE = 100 # set to 1 if running; TODO: take it out entirely when room is finalized
...
TRANSITION_SETS = ((6, 13, 11, 7, 0), (9, 2, 14, 10), (8, 4, 15), (16, 3), (12, 5), (1, 17, 18))
...
Tracking = None
if ROOM == "DEV":
		viz.MainView.setPosition([-0.6383535861968994, 1.4290522336959839, -3.6809496879577637])
		isense = viz.add('intersense.dle')
		headTracker = isense.addTracker(port=1)
		vizact.onkeydown('r',headTracker.resetHeading)
		headPos = viz.link(headTracker, viz.MainView, priority = 0)
...
def faceView():
	for i in range(len(avatars)):
		if (heads_state[i] != H_TRANS):
			...
			times[i] = 0
			heads[i].setEuler(heads[i].getEuler())
...
def nodAvatar(avatarNum):
	...
	if (avatarNum < len(avatars)):
		...
		yield viztask.waitTime(1)
		nodUp = [angle[0], angle[1] + 10, angle[2]]
		look = vizact.boneSpinTo('Bip01 Head',mode=viz.AVATAR_LOCAL,euler=nodUp,speed=100)
		avatars[avatarNum].runAction(look)
		yield viztask.waitTime(random.random()/2)
		look = vizact.boneSpinTo('Bip01 Head',mode=viz.AVATAR_LOCAL,euler=angle,speed=20)
		avatars[avatarNum].runAction(look)
